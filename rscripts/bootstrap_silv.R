...
for(case in split(CASES, 1:nrow(CASES))){
  ...
  boot_reps <- as.data.frame(do.call(rbind, mclapply(1:NUM_REPS_SIM, mc.cores = NUM_CORES, function(x){
    ...
    repeat{
      ...
      bw <- try(sqrt(diag(kernelboot::bw.silv(X_orig))), silent = TRUE)
      
      if(is.numeric(bw)){
        if(length(bw) == ncol(X_orig)){
          break
        }
      }
    }
    ...
    pbetti_boot <- replicate(NUM_REPS_BOOT, {
      ...
      PBETTI(RIPSER(GEN.BOOT(X_orig, bw), t/mult, q), r/mult, s/mult, q)
    })
  ...
  write.csv(boot_reps, sprintf('pbetti/reps/boot_reps_silv_f%d_q%d_n%d.csv', f, q, n), row.names = FALSE)
}
